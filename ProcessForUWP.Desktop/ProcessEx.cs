...
namespace ProcessForUWP.Desktop
{
    internal class ProcessEx : IDisposable
    {
                        ...
                        switch (message.MessageType)
                        {
                            ...
                            case MessageType.PropertyGet:
                                try
                                {
                                    ...
                                    Communication.SendMessages(nameof(ProcessEx), MessageType.PropertySet, message.ID, value);
                                }
                                catch (Exception ex)
                                {
                                    Debug.Write(ex);
        ...
        {
            Communication.SendMessages(nameof(ProcessEx), MessageType.ProcessErrorData, CommunicationID, e.Data);
        }

        private void Process_OutputDataReceived(object sender, DataReceivedEventArgs e)
        {
            Communication.SendMessages(nameof(ProcessEx), MessageType.ProcessOutputData, CommunicationID, e.Data);
        }
        ...
        private void Process_Exited(object sender, EventArgs e)
        {
            Process.OutputDataReceived -= Process_OutputDataReceived;
            Process.ErrorDataReceived -= Process_ErrorDataReceived;
            Communication.SendMessages(nameof(ProcessEx), MessageType.ProcessExited);
            Process.Exited -= Process_Exited;
        }
